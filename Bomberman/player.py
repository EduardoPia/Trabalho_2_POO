...
class Player:
    ...
    def atualizar(self, blocos:List[List[int]], orientacao:Vetor2D, bombas:List[Bomb]):
...
###### FALTA MEXER ######
        # for bomba in bombas:
        #         max_x = self.position.x + ConfigJogo.PLAYER_SIZE.x
        #         min_x = self.position.x - ConfigJogo.TILE_SIZE.x
        #         max_y = self.position.y + ConfigJogo.PLAYER_SIZE.y
        #         min_y = self.position.y - ConfigJogo.TILE_SIZE.y
        #         if (pos_x > max_x) and \
        #             (pos_x < min_x) and \
